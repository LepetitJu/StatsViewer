...
function render(tracks) {
  ...
  tracks.forEach(t => {
    ...
    b.className = 'result';
    b.innerHTML = `<img src="${esc(t.image)}" alt=""><div><h3>${esc(t.name)}</h3><p>${esc(t.artist)} · ${Number(t.listeners).toLocaleString('fr-FR')} auditeurs</p></div>`;
...
async function load(artist, track) {
  try {
    const r = await fetch(`/api/track?artist=${encodeURIComponent(artist)}&track=${encodeURIComponent(track)}`),
          d = await r.json();

    if (!r.ok) throw Error(d.error || 'Erreur serveur');

    document.querySelector('#cover').src = d.image || '';
    document.querySelector('#title').textContent = d.name;
    document.querySelector('#artist').textContent = d.artist;
    document.querySelector('#link').href = d.url || '#';
    document.querySelector('#listeners').textContent = d.listeners.toLocaleString('fr-FR');
    document.querySelector('#plays').textContent = d.playcount.toLocaleString('fr-FR');
